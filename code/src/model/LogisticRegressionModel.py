...
class LogisticRegressionModel(AbstractModel):
    ...
    def fit(self, train_x: np.ndarray, train_y: np.ndarray, grid_search: bool, run_info: dict) -> None:
        if grid_search:
            ...
            grid_result = grid_search_cv.fit(train_x, train_y)
            self._best_estimator = grid_result.best_estimator_
            pred = self._best_estimator.predict(train_x)
            ...
            run_info["training"] = {
                ...
                "fitted_model": {
                    "params": self._best_estimator["model"].get_params(),
                    "scores": {
                        "accuracy": scores[0],
                        "recall": scores[1],
                        "precision": scores[2],
    ...
    def predict(self, test_x: np.ndarray) -> np.ndarray:
        return self._best_estimator.predict(test_x)
