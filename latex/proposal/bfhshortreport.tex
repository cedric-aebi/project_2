...
for example when I'm currently writing this proposal. But longer lasting stress
or more intense stress can lead to mental and health problems like depression or
anxiety. For this reason, it makes sense to help monitor stress to try to
minimize the negative effects of long-time stress. With the advancements in
wearable technologies, smart gadgets like the Apple Watch can help to detect
stress with their onboard sensors. Machine Learning can then be applied to the
collected data from the smart devices to predict stress. In this work, Federated
Learning, as a way to do privacy-preserving machine learning, will be analyzed
and compared to traditional machine learning to predict stress on an already
collected stress-predict dataset.
...
photoplethysmogram (PPG) sensor. The raw PPG signal was then also filtered to
obtain a clean blood volume pulse (BVP) signal, which was then passed to and
estimation algorithm for the heart rate (HR) and inter-beat intervals (IBI)
...
metrics as individual or centralized learning.
...
test, interview session and hyperventilation session) with baseline/relaxation
...
a collective training. For this a public available FL framework like TensorFlow
...
on all the available data in the dataset. This scenario servers as a direct
comparison to FL, to see if FL looses any performance, while maintaining its
...
The datasets will be split into part to simulate individual clients and then
each client will do an 80\%-20\% train-test split on its available data. As
evaluation metrics, the F1 score, precision, recall and the accuracy will be
used. For the federated learning approach, both client-side evaluation (the
performance will be measured on the client-side on its test data) and
server side evaluation (the
performance will be measured on the server-side on its global held test data) will be done.

\section{Expected Results}
Theoretically: Since FL has the ability to see all the available data from all
...
centralized trained approach. The local trained models should perform
significantly worse compared both to the FL and centralized trained approach,
since the local devices have only a fraction of the data available and therefore
the models should underfit. But Fauzi et al.~\cite{fauzi_comparative_2022}
showed in their paper, that FL underperformed on the WESAD dataset. So it's hard
to say which results will come out. A good fitting model has to be chosen.

\section{Milestones}
\begin{enumerate}
  \item April 12, 2024 \textemdash{} Hand-in final proposal
  \item May 05, 2024 \textemdash{} First implementation with done with simple
