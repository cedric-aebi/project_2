...
WINDOW_LENGTH = 960  # 30 seconds at 32 Hz
STEP_SIZE = 32  # 1 second at 32 Hz
...
if __name__ == "__main__":
    ...
    for participant in tqdm(participants, desc=" Participants", position=0):
        ...
        for segment_id in tqdm(participant_df["segment_id"].unique(), desc=" Segments", position=1, leave=False):
            ...
            if len(segment_df) < WINDOW_LENGTH:
                continue

            eda_train, eda_val, eda_test = feature_service.split_and_window(
                data=segment_df["EDA"].values, window_length=WINDOW_LENGTH, step_size=STEP_SIZE
            )
            temp_train, temp_val, temp_test = feature_service.split_and_window(
                data=segment_df["TEMP"].values, window_length=WINDOW_LENGTH, step_size=STEP_SIZE
            )
            hr_train, hr_val, hr_test = feature_service.split_and_window(
                data=segment_df["HR"].values, window_length=WINDOW_LENGTH, step_size=STEP_SIZE
            )
            acc_x_train, acc_x_val, acc_x_test = feature_service.split_and_window(
                data=segment_df["X"].values, window_length=WINDOW_LENGTH, step_size=STEP_SIZE
            )
            acc_y_train, acc_y_val, acc_y_test = feature_service.split_and_window(
                data=segment_df["Y"].values, window_length=WINDOW_LENGTH, step_size=STEP_SIZE
            )
            acc_z_train, acc_z_val, acc_z_test = feature_service.split_and_window(
                data=segment_df["Z"].values, window_length=WINDOW_LENGTH, step_size=STEP_SIZE
            )

            splits = [
                ("train", eda_train, temp_train, hr_train, acc_x_train, acc_y_train, acc_z_train),
                ("val", eda_val, temp_val, hr_val, acc_x_val, acc_y_val, acc_z_val),
                ("test", eda_test, temp_test, hr_test, acc_x_test, acc_y_test, acc_z_test),
            ]

            for split_name, eda_windows, temp_windows, hr_windows, acc_x_windows, acc_y_windows, acc_z_windows in tqdm(
                splits, desc=" Splits", position=2, leave=False
            ):
                if eda_windows is None:
                    continue
                # Iterate over all the windows
                for i in tqdm(range(len(eda_windows)), desc=" Windows", position=3, leave=False):
                    ...
                    _, HR_cD_3, HR_cD_2, HR_cD_1 = pywt.wavedec(
                        hr_windows[i], "Haar", level=3
                    )  # 3 = 1Hz, 2 = 2Hz, 1=4Hz
                    ...
                    row = {
                        ...
                        "Split": split_name,
                        "Label": label,
                        **results.to_dict(orient="records")[0],
                    }
